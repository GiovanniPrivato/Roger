...
class Obfuscator
{
    ...
    public static function obfuscateFile(string $file, string $newFile){
        if(!file_exists($file)) throw new Exception("File " . $file . " does not exist");
        if(file_exists($newFile)) unlink($newFile);

        $code = file_get_contents($file);
        file_put_contents($newFile, self::obfuscateCode($code));
    }

    public static function obfuscateCode(string $code, int $noiseLevel = 12) : string {
