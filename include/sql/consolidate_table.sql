IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('[%s].[%s]'))
exec('CREATE PROCEDURE [%s].[%s] AS BEGIN SET NOCOUNT ON; END')	
GO

ALTER PROCEDURE [%s].[%s]
@table_name varchar(255),
@final_table varchar(255),
@field_1 varchar(255),
@field_2 varchar(255),
@field_3 varchar(255),
@field_4 varchar(255),
@field_5 varchar(255),
@use_column_names BIT
AS
BEGIN
--***************** THIS PROCEDURE IS AUTO GENERATED BY ROGER - PLEASE DO NOT EDIT THIS PROCEDURE **********************--
SET NOCOUNT ON;
DECLARE @table_cursor Cursor,
		@c_table varchar(255),
		@sql nvarchar(max),
		@columns NVARCHAR(MAX),
		@final_columns NVARCHAR(MAX),
		@missing_columns NVARCHAR(MAX)

	SET @table_cursor = Cursor For
	SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES 
	WHERE TABLE_SCHEMA = 'dbo' 
	AND TABLE_TYPE = 'BASE TABLE'
	AND TABLE_NAME LIKE @table_name
	AND TABLE_NAME <> @final_table
	
	CREATE TABLE #metaTable(META_FIELD_1 varchar(255) COLLATE DATABASE_DEFAULT, META_FIELD_2 varchar(255) COLLATE DATABASE_DEFAULT, META_FIELD_3 VARCHAR(255) COLLATE DATABASE_DEFAULT, META_FIELD_4 VARCHAR(255) COLLATE DATABASE_DEFAULT, META_FIELD_5 VARCHAR(255) COLLATE DATABASE_DEFAULT)

	OPEN @table_cursor

	Fetch Next From @table_cursor
	Into @c_table

	While (@@FETCH_STATUS = 0)
		BEGIN
			
				IF (NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES 
				WHERE TABLE_SCHEMA = 'dbo' 
				AND  TABLE_NAME = @final_table)) 
				BEGIN
					exec('SELECT * INTO [' + @final_table + '] from [' + @c_table + ']')
				END 
				ELSE 
				BEGIN
					TRUNCATE TABLE #metaTable
					
					INSERT INTO #metaTable(META_FIELD_1, META_FIELD_2, META_FIELD_3, META_FIELD_4, META_FIELD_5)
					exec('SELECT distinct ' + @field_1 + ',  ' + @field_2 + ', ' + @field_3 + ', ' + @field_4 + ', ' + @field_5 +' FROM [' + @c_table + ']')

					SET @sql = N'DELETE F
								FROM [' + @final_table + '] AS F
								INNER JOIN #metaTable AS M
									ON ' + @field_1 + ' = M.META_FIELD_1
								AND ' + @field_2 + ' = M.META_FIELD_2
								AND ' + @field_3 + ' = M.META_FIELD_3
								AND ' + @field_4 + ' = M.META_FIELD_4
								AND ' + @field_5 + ' = M.META_FIELD_5;
							';
							
							EXEC sp_executesql @sql;

							---------------------------------------------
							-- SELECT *
							IF @use_column_names = 0
							BEGIN
								SET @sql = '
									INSERT INTO [' + @final_table + ']
									SELECT *
									FROM [' + @c_table + '];';
									EXEC sp_executesql @sql
							END
							ELSE
							BEGIN
								-- Build column list based on destination table
								-- keeping only columns available in source table
								SELECT @final_columns =
									STRING_AGG(CONVERT(NVARCHAR(max), QUOTENAME(f.name)), ', ')
									WITHIN GROUP (ORDER BY f.column_id)
								FROM sys.columns f
								WHERE f.object_id = OBJECT_ID('dbo.[' + @final_table + ']')
								AND f.is_computed = 0
								AND f.is_identity = 0;

								SELECT @columns =
									STRING_AGG(CONVERT(NVARCHAR(max), QUOTENAME(f.name)), ', ')
									WITHIN GROUP (ORDER BY f.column_id)
								FROM sys.columns f
								INNER JOIN sys.columns c
									ON c.name = f.name
								WHERE f.object_id = OBJECT_ID('dbo.[' + @final_table + ']')
								AND c.object_id = OBJECT_ID('dbo.[' + @c_table + ']')
								AND f.is_computed = 0
								AND f.is_identity = 0;

								-- Safety check
								IF (@columns IS NULL) OR (LEN(@columns) = 0) OR (LEN(@final_columns) <> LEN(@columns))
								BEGIN
									SELECT @missing_columns =
										'Missing columns ' + STRING_AGG(CONVERT(NVARCHAR(max), QUOTENAME(f.name)), ', ')
										WITHIN GROUP (ORDER BY f.column_id)
									FROM sys.columns f
									LEFT OUTER JOIN sys.columns c
										ON c.name = f.name AND c.object_id = OBJECT_ID('dbo.[' + @c_table + ']')
									WHERE f.object_id = OBJECT_ID('dbo.[' + @final_table + ']')
									AND f.is_computed = 0
									AND f.is_identity = 0
									AND c.name IS NULL;

									THROW 50001, @missing_columns, 1;
								END;

								ELSE 
								BEGIN
									SET @sql = '
										INSERT INTO [' + @final_table + ']
										(' + @columns + ')
										SELECT ' + @columns + '
										FROM [' + @c_table + ']';
									EXEC sp_executesql @sql
								END
							END

							-- Debugging (optional)
							--PRINT @sql;

							
							--exec('DROP TABLE [' + @c_table + ']')
						END
							---------------------------------------------
							--exec('INSERT INTO [' + @final_table + '] SELECT * FROM [' + @c_table + ']')
				--END
				
			Fetch Next From @table_cursor Into @c_table
		END

		CLOSE @table_cursor 
	DEALLOCATE @table_cursor

END

