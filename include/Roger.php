...
class Roger
{
    ...
    public function upload2SQL(string $file, string $table)
    {
        ...
        try {
            ...
            $this->logger->writelog("FILE - $file", 'SUCCESS');

        } catch (Exception $e) {
            $this->logger->writelog("FILE - " . $e->getMessage());
        } finally {
            $this->disconnect();
        }
    }
    ...
    public function runBatchFiles()
    {
        ...
        foreach ($scripts as $s) {
            ...
            try {
                $res = shell_exec($s);
                $this->logger->writelog("BATCH - " . basename($s), 'SUCCESS');
            } catch (Exception $e) {
                $this->logger->writelog("BATCH - " . basename($s) . " - " . $e->getMessage());
            }
    ...
    private function getAndSanitizeHeader(string $file)
    {
        ...
        $headerFields = explode($this->fieldseparator, $headerLine);
        $headerFields = array_map(fn($h, $i) => $h ? $h : "Field$i", $headerFields, array_keys($headerFields));

        $headerLine = implode($this->fieldseparator, $headerFields);
    ...
    {
        $encoding = mb_detect_encoding($field, ['ISO-8859-1', 'ASCII', 'UTF-8']) ?? mb_detect_encoding($field);
        if ($encoding) {
            $field = mb_convert_encoding($field, "UTF-8", $encoding);
        }

        $string         = preg_replace('/[^\p{L}\p{N}_]/u', '', $field); //remove special chars.
